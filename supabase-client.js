...
const Auth = {
  ...
  async register(employeeId, name, role, password, unit, lang){
    if(!['care','social','nurse','foreign_care'].includes(role)) throw new Error('角色不正確');
    ...
    const { error: pErr } = await sb.from('profiles').insert({
      id: uid, employee_id: String(employeeId).toUpperCase(), name, role,
      lang: role==='foreign_care' ? (lang||'vi') : 'zh',
      unit: unit || '', must_change_password: false,
    });
    if(pErr){
      await sb.auth.signOut();
